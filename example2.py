...
with tf.Session('', config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
    ...
    verts = tf.Variable(np.expand_dims(vertices, axis=0))
    ...
    image_ref = scipy.misc.imread('data/example2_ref.png').astype('float32').mean(-1) / 255.
    ...
    make_gif(working_directory, 'result/example2_optimization.gif')
    ...
    # generate gif (need ImageMagick)
    options = '-delay 8 -loop 0 -layers optimize'
    subprocess.call('convert %s %s/_tmp_*.png %s' % (options, working_directory, 'result/example2.gif'), shell=True)
