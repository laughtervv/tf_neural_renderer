...
with tf.Session('', config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
    ...
    elevation = 30
    texture_size = 2
    textures = tf.Variable(np.zeros((1, faces.shape[0], texture_size, texture_size, texture_size, 3), 'float32'))
    ...
    image_ref = scipy.misc.imread('data/example3_ref.png').astype('float32') / 255.
    ...
    train = tf.train.AdamOptimizer(0.1, beta1=0.5).minimize(loss)
    ...
    make_gif(working_directory, 'result/example3_optimization.gif')
    ...
    # generate gif (need ImageMagick)
    options = '-delay 8 -loop 0 -layers optimize'
    subprocess.call('convert %s %s/_tmp_*.png %s' % (options, working_directory, 'result/example3.gif'), shell=True)
